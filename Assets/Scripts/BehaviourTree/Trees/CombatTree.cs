...
public class CombatTree : BehaviourTree
{
    ...
    public CombatTree(Blackboard pBlackboard, int pPriority = 0) : base("Combat", pPriority)
    {
        ...
        blackboard.TryGetValue(CommonKeys.FindRadius, out float radiusValue);
        findRadius = radiusValue;
    ...
    void setup()
    {
        ...
        Leaf _findEnemiesAction = new Leaf("Combat/FindTargets", new FindEnemiesStrategy(blackboard, agent.transform,findRadius));
        Leaf _obtainEnemy = new Leaf("Combat/ObtainTarget", new ChooseEnemyStrategy(blackboard));
        ...
        Leaf _findAllies = new Leaf("Combat/TargetSeq/RandSel/FindAllies", new FindAlliesStrategy(blackboard, agent.transform, findRadius));
